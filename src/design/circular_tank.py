...
def design_circular_tank(
    ...
    ct, cm, param = get_ct_cm(H_total, D, t_wall)
    ...
    comp.details = {
        ...
        "Ast_hoop_prov": hoop_bar["Ast_prov"],
    ...
    if hoop_bar["Ast_prov"] < Ast_hoop:
        comp.fail(f"Hoop steel deficient: prov={hoop_bar['Ast_prov']:.0f} < req={Ast_hoop:.0f} mm²/m")
    result.add(comp)
