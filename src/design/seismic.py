...
def seismic_forces(
    ...
    # Units: mass in tonnes (Mg) and stiffness in kN/m give m_t / K_s directly
    # in seconds² (1 t = 1000 kg, 1 kN/m = 1000 N/m, so the 1000s cancel).
    m_i_total_t = (mi + ms)        # tonnes
    T_i = 2 * math.pi * math.sqrt(m_i_total_t / K_s)  # sec
    T_i = max(T_i, 0.03)
